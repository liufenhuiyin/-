...
class ContextPanel(ft.Container):
    """
    上下文管理面板（用 ft.Container 实现右侧覆盖抽屉效果）。

    公开接口：
        show()                              — 显示面板
        hide()                              — 隐藏面板
        load_blocks(items: list)            — 加载 ContextBlockVM 列表
        load_templates(items: list)         — 加载 TemplateVM 列表
        set_preview(text: str)              — 更新拼接预览文本
    """
    ...
    def __init__(
        ...
        on_add_text_block: Callable[[str], None],
                    ...
                    ft.Row(
                        controls=[
                            ...
                            ft.IconButton(
                                ...
                                icon_size=20,
                                tooltip="添加",
                                on_click=lambda e: self._handle_add_block(
                                    on_add_text_block
                                ),
                            ),
                        ],
    ...
    def load_blocks(self, items: list) -> None:
        """接收 list[ContextBlockVM]，预期字段: id, label, preview, enabled"""
        ...
        for vm in items:
            self._blocks_list_ref.current.controls.append(
                _ContextBlockItem(
                    ...
                    on_remove=lambda bid: None,   # 由外部绑定
                    on_toggle=lambda bid, v: None,
                )
            )
        self._blocks_list_ref.current.update()

    def load_templates(self, items: list) -> None:
        """接收 list[TemplateVM]，预期字段: id, name, description"""
        ...
        for vm in items:
            self._templates_list_ref.current.controls.append(
                _TemplateItem(
                    ...
                    on_apply=lambda tid: None,    # 由外部绑定
                )
            )
