...
class _ModelSelector(ft.Container):
    ...
    def _handle_change(self, e: ft.ControlEvent) -> None:
        app_config.model_type = e.control.value  # 直接写入集中配置
        if self._on_change:
        ...
        chip = ft.Container(
            content=ft.Row(
                controls=[
                    ...
                    ft.Text(name, size=Fonts.SIZE_XS, color=Colors.TEXT_SECONDARY,
