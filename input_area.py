...
class _AttachmentBar(ft.Row):
    def __init__(self) -> None:
        ...
        self._files: list[tuple[str, str]] = []  # (name, path)

    def add_file(self, name: str, path: str = "") -> None:
        self._files.append((name, path or name))
        chip = ft.Container(
            content=ft.Row(
                controls=[
...
class InputArea(ft.Container):
    ...
    def _on_file_pick(self, e) -> None:
        if not e.files:
            return
        for f in e.files:
            # f.path 是绝对路径，f.name 是文件名
            self._attachment_bar.add_file(f.name, getattr(f, 'path', f.name) or f.name)
